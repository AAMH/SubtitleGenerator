...
/**
 * Created by AmirHossein on 10/26/15.
 */
public class MyViewer extends MediaListenerAdapter implements IMediaListener, IMediaViewer {

    private static int MyArbitraryWidth = 700;
    private static int MyArbitraryHeight = 500;

    private static int LastUsedSub = 0;

    private static final Logger log = LoggerFactory.getLogger(MyViewer.class);
    ...
    MyViewer(Mode mode, boolean showStats)
    {
    ...
    MyViewer(boolean showStats, int defaultCloseOperation)
    {
    ...
    private static void drawStats(IVideoPicture picture, BufferedImage image)
    {
        ...
        g.drawString(getProperSubtitle(timeStamp), 100, 0);
    }

    private static String getProperSubtitle(String timeStamp){
        for(int r = 0;r<GUI.Subs.size();r++) {
            int i = Integer.parseInt(GUI.Subs.get(r).getStartTime().replaceAll(":", "").replaceAll(",", ""));
            int j = Integer.parseInt(GUI.Subs.get(r).getEndTime().replaceAll(":", "").replaceAll(",", ""));
            int k = Integer.parseInt(timeStamp.replaceAll(":", "").replaceAll("\\.", ""));

            //  if(k > j)
            //     LastUsedSub++;
            if (k >= i && k <= j) {
          //      LastUsedSub = r;
                return GUI.Subs.get(r).getText();
            }
        }
    ...
    private class MediaFrame extends PositionFrame
    {
        ...
        protected void setVideoImage(IVideoPicture picture, BufferedImage image)
        {
            ...
            if (null != image)
            {
                ...
                {
                    setVideoSize(new Dimension(MyArbitraryWidth, MyArbitraryHeight));
                }
                repaint();
            }
        }
        ...
        protected void paintPanel(Graphics2D graphics)
        {
            ...
            if (mImage != null)
                graphics.drawImage(mImage, 0, 0,MyArbitraryWidth,MyArbitraryHeight, null);
        }
    }
    ...
    private static class StatsFrame extends PositionFrame
    {
        ...
        private final MyViewer mViewer;
        ...
        public StatsFrame(int defaultCloseOperation, MyViewer viewer)
        {
            super(defaultCloseOperation);
